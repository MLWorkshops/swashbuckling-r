...
We are attracted to edges in visual media, seeing contrasts and brightness relatively rather than in the absolute. Thus background choices matter - the same color or shade will be perceived differently against a darker background vs. a ligher one. Here we, plot the points against a light grey and dark grey background.
...
p1 = ggplot(pirate_data, aes(x=x, y=y)) +
  geom_point()
...
p2 = ggplot(pirate_data, aes(x=x, y=y)) +
...
plot1 = ggplot(data, aes(x=x, y=y, shape=color)) +
